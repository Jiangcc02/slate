...
- **完整的数字学校**：覆盖教、学、练、测、评、管、营全链路——9 个业务域（招生 / 课程 / 学习 / 测评 / 学情档案 / 教务 / 家校 / 收费 / 校园服务）+ 平台底座，六端触达（学生 / 教师 / 家长 / 教室白板 / 教务 / 校长）
...
**人负责设计，Agent 负责实现。**协调者先定契约、搭好平台底座与工程骨架；此后每位成员认领自己的系统 / 模块，把精力花在想清楚它该是什么样——要哪些能力、完成时用户看到什么、对外契约长什么样，写成蓝图与设计文档。各自的 Code Agent 领取任务，把设计变成可运行的代码，并承担全部后续迭代：实现、自测、按评审意见修改、修复对齐偏差——对 Agent 来说，重做一遍只是几分钟的事。

- **一条主线，各自侧枝**：所有人从 `dev` 主线拉自己的分支并行推进；模块「能跑」（冒烟用例通过）即由该成员的 Agent 发 PR，合回主线
- **一个模块是一条完整纵切**：后端服务 + 对应端的界面与用户体验，由同一位成员设计、同一个 Agent 交付——不是「后端能跑就行」；除非任务书明确只切一块（如单独的前端 UI 设计、后端性能优化），默认都是完整的前后端设计
- **边界之内完全自由**：模块内怎么设计是设计者的事，风格不受评审；接缝上的分歧（数据归属、接口假设、要不要改底座）由协调者仲裁，裁定写入契约、全员可见
- **机器守红线，人守方向**：CI 自动检查契约与依赖红线，PR 经人批准后才合入（见下方「治理特色」）
...
| 期 | 目标 | 范围 |
|---|---|---|
| 一期 | **能上课** | 平台底座 + 课程 + 开班 + 学习中心 + 任务型作业闭环（拍照/文本提交，不依赖题库） |
| 二期 | **能考试** | 题库、组卷、三形态测评（含题库型作业）、判分、成绩排名、错题本 |
| 三期 | **有洞察、管好学校** | 学情分析/画像/预警/全校数据总览；教务本体（课表/排课/考务/学籍）；家校协同 |
| 四期 | **按定位选配** | 教室端（白板）、教学设备报修工单（底座设备管理）、招生（私立）、收费（含饭卡充值/代收消费账单）、教师工作量、答疑讨论、直播、移动端、售卖 |
| 五期 | **校园事务数字化（按需）** | 校园服务域：行政办公（OA：公文/审批/会议）、后勤事务（设施报修/巡检）、宿舍门禁与一卡通对接 |
| Agent 建设期 | **数智化** | 前置条件：系统生产稳定运行一段时间（启动判据待裁定） |
