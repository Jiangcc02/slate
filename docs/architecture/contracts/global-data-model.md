...
status: v0.2-draft（主链与支链锚点级骨架；ID 契约已定稿；列级设计归各域设计文档，本文件只锁 ID 契约与关系骨架）
...
- 类型 int64；**生成策略已定稿（底座设计 §3.1，2026-09-09）：雪花 ID**（41 位毫秒时间戳 + 10 位 workerId + 12 位序列，epoch=2025-01-01，workerId 全系统唯一配置）；统一生成器为 slate-framework `SnowflakeIdGenerator`，全系统统一使用
...
| v0.2-draft | 2026-09-09 | ID 生成策略定稿：雪花 ID（底座设计 §3.1 回填，生成器已随工程骨架落地 slate-framework） |
